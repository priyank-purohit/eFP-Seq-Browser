...
for region in map_info[u'result']:
	if region[u'strand'] == u'+':
		if region[u'start'] < start:
			...
			mRNA["start"].append(int(items['start']))
		if region[u'end'] > end:
			end = region[u'end']
			mRNA["end"].append(int(items['end']))
	else:
		if region[u'start'] < start:
			start = region[u'start']
			mRNA["start"].append(int(items['start']))
...
def generate_rnaseq_graph(urlx, filename, out_clr):
	...
	try:	
 		with open("mpileups/"+geneid+"/"+urlx, 'r') as file:
            		line = file.readline().split()
			for i in range(mRNA["start"][0], mRNA["end"][0]):
                		if line != []:
                    			if int(line[1]) == i:
                        			expression_score.append(int(line[3]))
                        			line = file.readline().split()
                    			else:
                        			expression_score.append(0)
                		else:
                    			expression_score.append(0)
		file.close()
...
def generate_PCC():
	...
	# Fine Standard
	def find_standard(exon, mRNA):
        		MAX = 100
        		standard = []
        		counter = 0
       		for i in range(mRNA["start"][0], mRNA["end"][0]):
            			if (i >= exon["start"][counter]) and (i <= exon["end"][counter]):
               				 standard.append(MAX)
           			else:
                			if i == (exon["end"][counter]+1):
                    				counter += 1
                				standard.append(0)
        	...
        	# Find Denominator
	def find_sum_difference(self, standard, expression_score):
        
        		standard_mean = self.find_mean (standard)
       		 expression_mean = self.find_mean (expression)
        
       		x_value = 0
        		y_value = 0
        
       		if len(standard) == len (expression):
            			for i in range(len(standard)):
                			x_value = x_value + (standard[i] - standard_mean)**2
                			y_value = y_value + (expression[i] - expression_mean)**2

        		denominator = math.sqrt(x_value*y_value)
        		return denominator 
        
	# Find Numerator
	def find_variance(self, standard, expression_score):
       		...
       		if len(standard) == len (expression):
            			for i in range(len(standard)):    
                			numerator = numerator + ((standard[i]-standard_mean)*(expression[i]-expression_mean))
